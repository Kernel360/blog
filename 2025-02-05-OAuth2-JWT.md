...
현재 CORS 정책을 강화하기 위해 .allowedOriginPatterns("")을 추가했습니다.
...
### ✅ CSRF 방어 적용
SameSite 설정 변경
.sameSite("Lax") 또는 .sameSite("Strict") 설정을 고려
CSRF 방어를 위해 가능하면 Lax 이상으로 설정
CSRF 토큰 사용
POST, PUT, DELETE 요청 시 CSRF 토큰을 추가 인증 수단으로 사용

### ✅ Refresh Token 및 블랙리스트 적용
...
CSRF 토큰 추가를 통해 로그인 보안성을 강화의 필요성을 느꼈고 빠르게 개선 해야겠다.
