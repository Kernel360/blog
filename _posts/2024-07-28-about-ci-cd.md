...
서로 다른 코드 스타일로 인해 코드가 조금씩 어지러워진다는 느낌이 들기도 하였습니다. 이는 프로젝트의 규모가 커지며 더 심각하게 느껴질 수 있는 부분일 것입니다. 따라서 저는 위와같은 어려움을 겪으며 CI(지속적 통합)와 CD(지속적 배포)의 필요성을 느끼게 되었습니다.
...
- **Git 사용**: 분산 버전 관리 시스템인 Git을 사용하여 코드 변경 사항을 효율적으로 관리합니다.
- **브랜치 전략**: Git Flow와 같은 브랜치 전략을 도입하여 코드 변경을 체계적으로 관리합니다.

**3.2 자동화된 빌드**

- **Jenkins**: 다양한 빌드 툴을 사용하여 코드를 자동으로 빌드하고, 빌드 실패 시 즉시 피드백을 받을 수 있습니다.
- **GitHub Actions**: GitHub 저장소에서 바로 빌드, 테스트, 배포 작업을 자동화할 수 있는 도구입니다.
- **빌드 스크립트 작성**: 각 프로젝트에 맞는 빌드 스크립트를 작성하여 빌드 과정을 자동화합니다.

**3.3 자동화된 테스트**

- **단위 테스트**: 각 모듈의 기능을 개별적으로 테스트하는 단위 테스트를 작성합니다.
- **통합 테스트**: 여러 모듈이 함께 작동하는지 확인하는 통합 테스트를 수행합니다.
- **자동 테스트 실행**: 코드 변경 시 자동으로 테스트가 실행되도록 설정하여, 테스트 통과 여부를 즉시 확인합니다.

**3.4 코드 스타일 검사**

- **ESLint**: JavaScript 코드의 스타일과 규칙을 검사하고 일관성을 유지하기 위한 도구입니다.
- **Stylelint**: CSS 코드의 스타일을 검사하고 일관성을 유지하기 위한 도구입니다.
- **GitHub Actions에서의 Lint 체크**: 코드 변경 시마다 자동으로 ESLint와 Stylelint를 실행하여 코드 스타일을 검사하고, 문제가 있는 경우 이를 바로 피드백합니다.

**3.5 자동화된 배포**

- **Vercel**: Vercel을 사용하여 배포를 자동화합니다. GitHub와 연동하여 코드 변경이 발생할 때마다 자동으로 빌드 및 배포가 이루어지도록 설정합니다.

**3.6 모니터링과 로깅**

- **모니터링 도구**: Prometheus, Grafana와 같은 모니터링 도구를 사용하여 애플리케이션의 상태를 실시간으로 모니터링합니다.
- **로그 관리**: ELK 스택(Elasticsearch, Logstash, Kibana)을 사용하여 로그를 수집, 분석하고, 문제 발생 시 신속히 대응할 수 있도록 합니다.
